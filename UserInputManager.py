...
class UserInputManager:
    ...
    def get_writing_style():
        ...
        style = st.selectbox("글쓰기 스타일을 선택해주세요:", styles, key="writing_style_selectbox")
        return style

    @staticmethod
    def get_writing_length():
        length = st.number_input(
            "원하는 글의 길이를 100자 단위로 입력해주세요 (최대 5000자):",
            min_value=100, max_value=5000, step=100, key="writing_length_number_input"
        )
        return int(length)
    ...
    def get_temperature():
        temperature = st.slider(
            "창의성 수준을 입력해주세요 (0 ~ 10, 0: 매우 일관된, 10: 매우 창의적):",
            0.0, 10.0, step=0.1, key="temperature_slider"
        )
        return temperature / 10

    @staticmethod
    def get_user_info():
        age = st.text_input("사용자의 나이를 입력해주세요:", key="user_age_text_input")
        gender = st.radio("사용자의 성별을 선택해주세요:", options=["남성", "여성"], key="user_gender_radio")
        
        tone_options = [f"{key}: {style} - {description}" for key, (style, description, _) in WRITING_TONES.items()]
        selected_tone = st.selectbox("문체를 선택해주세요:", tone_options, key="user_tone_selectbox")
    ...
    def confirm_action(prompt):
        return st.radio(prompt, options=["y", "n"], key=f"{prompt}_radio") == 'y'

    @staticmethod
    def get_user_choice(prompt, options):
        choice = st.selectbox(f"{prompt} ({'/'.join(options)})", options, key=f"{prompt}_selectbox")
        return choice